...
import com.amachan.listview.R;
import com.amachan.swipe.SwipeHelper2;
import com.amachan.swipe.SwipeHelper2.OnItemGestureListener;
import com.amachan.swipe.SwipeHelper2.SwipeHelperCallback;

/**
 * The ListView composed of {@link ContactTileRow}. This ListView handles both -
 * Swiping, which is borrowed from packages/apps/UnifiedEmail
 * (com.android.mail.ui.Swipeable) - Drag and drop
 */
public class SwipeableListView extends ListView implements SwipeHelperCallback {
    ...
    private SwipeHelper2 mSwipeHelper;
    ...
    // child's left coordinate. This is used to aid in the drawing of the drag
    // shadow.
    ...
    /**
     * {@link #mTopScrollBound} and {@link mBottomScrollBound} will be offseted
     * to the top / bottom by {@link #getHeight} * {@link #BOUND_GAP_RATIO}
     * pixels.
     */
    ...
    public SwipeableListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mSwipeHelper = new SwipeHelper2(context, SwipeHelper2.X, this,
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            Log.d("Anoosh", "onInterceptTouchEvent - ACTION_DOWN");
        } else if (ev.getAction() == MotionEvent.ACTION_MOVE) {
            Log.d("Anoosh", "onInterceptTouchEvent - ACTION_MOVE");
        }
    ...
    public View getChildAtPosition(MotionEvent ev) {
        ...
        if (view != null &&
                SwipeHelper2.isSwipeable(view) &&
                view.getVisibility() != GONE) {
            // If this view is swipable in this listview, then return it.
            // Otherwise
            // return a null view, which will simply be ignored by the swipe
            // helper.
            return view;
        }
        return null;
    ...
    public boolean dispatchDragEvent(DragEvent event) {
        ...
        switch (action) {
            ...
            case DragEvent.ACTION_DRAG_LOCATION:
                ...
                if (!mIsDragScrollerRunning &&
                        // And if the distance traveled while dragging exceeds
                        // the touch slop
        ...
        // This ListView will consumer the drag events on behalf of its
        // children.
        return true;
    }
    ...
    private boolean handleDragStarted(int x, int y) {
        ...
        /*
         * if (tile.getTileAdapter().hasPotentialRemoveEntryIndex()) { return
         * false; }
         */

        final int itemIndex = this.getPositionForView(child);
        if (child != null && mOnDragDropListener != null) {
            /*
             * final PhoneFavoriteTileView tileView = (PhoneFavoriteTileView)
             * tile.getViewAtPosition(x, y);
             */
            ...
            mDragShadowBitmap = createDraggedChildBitmap(child);
            if (mDragShadowBitmap == null) {
                return false;
            }
             ...
             * mDragShadowLeft = tile.getLeft(); mDragShadowTop = tile.getTop();
             * } else {
             */
            // Square tile is relative to the contact tile,
            // and contact tile is relative to this list view.
            mDragShadowLeft = child.getLeft();
            mDragShadowTop = child.getTop();
            // }
            ...
            // x and y passed in are the coordinates of where the user has
            // touched down, calculate
            // the offset to the top left coordinate of the dragged child. This
            // will be used for
    ...
    private void handleDragHovered(int x, int y) {
        ...
        // Draw the drag shadow at its last known location if the drag shadow
        // exists.
        if (mDragShadowOverlay != null) {
            mDragShadowOverlay.setX(mDragShadowLeft);
            mDragShadowOverlay.setY(mDragShadowTop);
        ...
        final View child = getViewAtPosition(x, y);
        int itemIndex = this.getPositionForView(child);
        /*
         * if (!(child instanceof ContactTileRow)) { // Bail early. return; }
         */

        /*
         * final ContactTileRow tile = (ContactTileRow) child; final int
         * itemIndex = tile.getItemIndex(x, y);
         */
        if (child != null && itemIndex != -1 && mOnDragDropListener != null) {
